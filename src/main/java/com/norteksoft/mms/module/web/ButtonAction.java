	...
	private String defaultTree(Long menuId){
		...
		if(children==null||children.size()<=0){
			...
			if(modulePages.size()>0){
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+"(菜单)", "true", "false", "", "", "folder", "");
				treeNodes.add(node);
				childModulePage(modulePages,menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes);
			}else{
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+"(菜单)", "false", "false", "", "", "folder", "");
				treeNodes.add(node);
			}
		}else{
			node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu","0",getInternation(menu.getName())+"(菜单)", "true", "false", "", "", "folder", "");
	...
	private void childMenu(List<Menu> menus,String pid,List<ZTreeNode> treeNodes,List<Long> systemIds){
		...
		for(Menu menu :menus){
			...
			if(children==null||children.size()<=0){
				List<ModulePage> modulePages=modulePageManager.getEnableModulePagesByMenuId(menu.getId());
				if(modulePages.size()>0){
					node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+"(菜单)", "true", "false", "", "", "folder", "");
					treeNodes.add(node);
					childModulePage(modulePages,menu.getId().toString()+";"+getInternation(menu.getName())+";menu",treeNodes);
				}else{
					node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+"(菜单)", "false", "false", "", "", "folder", "");
					treeNodes.add(node);
				}
			}else{
				node = new ZTreeNode(menu.getId().toString()+";"+getInternation(menu.getName())+";menu",pid,getInternation(menu.getName())+"(菜单)", "true", "false", "", "", "folder", "");
