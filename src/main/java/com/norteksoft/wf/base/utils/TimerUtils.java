...
public class TimerUtils {
	...
	public void swingInform(WorkflowTask task,Set<String> transactorIds) throws Exception {
		for(String userId : transactorIds){
			...
			String msg = new StringBuilder(getInterationValue("inform.message.info.workflow", language)+":"+task.getGroupName()).append(",").append(getInterationValue("inform.message.info.task", language)+":").append(task.getName()).append(",").append(getInterationValue("inform.message.info.title", language)+":").append(task.getTitle()).append(getInterationValue("inform.message.info.part1", language)).append(task.getTransactorName()).append(getInterationValue("inform.message.info.part2", language)).toString();
			com.norteksoft.product.api.entity.User user = ApiFactory.getAcsService().getUserById(Long.parseLong(userId));
			if(user!=null){
				Long messageId = ApiFactory.getPortalService().addMessage("task", "系统管理员", ContextUtils.getUserId(), user.getId(),"催办超期提醒", msg, "/task/workflow-notification.htm?notificationType=remind&id="+task.getId(),true,"task--"+task.getProcessInstanceId());
	...
	private String getInterationValue(String code,String language){
		return ApiFactory.getSettingService().getInternationOptionValue(code, language, InternationType.WORKFLOW_RESOURCE.toString());
	}
	...
	public void emailReminder(WorkflowTask task){
		...
		.append(getTaskDay(task)).append("天了。请尽快办理！").toString();
	...
	public void swingReminder(WorkflowTask task) throws Exception{
		if(StringUtils.isNotEmpty(task.getTransactor())){
			...
			String msg = new StringBuilder("(").append(new SimpleDateFormat("yyyy-MM-dd").format(new Date(System.currentTimeMillis()))).append( ")"+getInterationValue("reminder.message.info.task", language)+"：").append(task.getTitle()).append( getInterationValue("reminder.message.info.part1", language))
					.append(getTaskDay(task)).append(getInterationValue("reminder.message.info.part2", language)).toString();
			if(task.getTransactorId()==null){
				ApiFactory.getPortalService().addMessage("task", "系统管理员", ContextUtils.getLoginName(), task.getTransactor(),"待办任务催办提醒", msg, "/task/message-task.htm?id="+task.getId(),true,"task-"+task.getId()+"-"+task.getProcessInstanceId());
			}else{
				ApiFactory.getPortalService().addMessage("task", "系统管理员", ContextUtils.getUserId(), task.getTransactorId(),"待办任务催办提醒", msg, "/task/message-task.htm?id="+task.getId(),true,"task-"+task.getId()+"-"+task.getProcessInstanceId());
