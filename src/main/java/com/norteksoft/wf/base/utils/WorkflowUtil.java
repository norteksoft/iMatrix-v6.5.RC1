		...
		if(taskId!=null){
			...
			if(transactNum>1){
				...
				for(com.norteksoft.product.api.entity.WorkflowTask currentTask:currentPrincipalTasks){
					if(!taskId.equals(currentTask.getId())){//减签时如果不是当前打开的任务
						String showName = currentTask.getTransactorName()+"（受托于"+currentTask.getTrustorName()+")";
						if(hasBranch){
							showName = currentTask.getTransactorName()+"/"+currentTask.getTransactorSubCompanyName()+"（受托于"+currentTask.getTrustorName()+"/"+currentTask.getTrustorSubCompanyName()+")";
						}
