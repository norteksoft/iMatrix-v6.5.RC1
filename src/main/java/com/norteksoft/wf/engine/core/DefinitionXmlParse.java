    	...
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				...
    				if(user!=null){
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),"待办任务流向通知",getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName,true,"task--"+workflow.getProcessInstanceId());
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),"待办任务流向通知",getNeedInformSubject(processId,transitionName), "/task/workflow-notification.htm?notificationType=transition&processId="+processId+"&transitionName="+transitionName,true,"task--"+workflow.getProcessInstanceId());
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
    	...
    	if(StringUtils.isNotEmpty(receivers)){
    		...
    		for(String loginName:loginNames){
    			try {
    				User user = UserUtil.getUserByInfo(loginName);
    				if(user!=null){
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getUserId(), user.getId(),"流程结束时通知",getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId,true,"task--"+workflow.getProcessInstanceId());
    				}else{
    					ApiFactory.getPortalService().addMessage("task", ContextUtils.getUserName(), ContextUtils.getLoginName(), user.getLoginName(),"流程结束时通知",getProcessInformSubject(processId), "/task/workflow-notification.htm?notificationType=process&processId="+processId,true,"task--"+workflow.getProcessInstanceId());
    				}
				} catch (Exception e) {
					e.printStackTrace();
				}
