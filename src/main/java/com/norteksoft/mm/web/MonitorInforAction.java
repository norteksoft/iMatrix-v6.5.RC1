...
public class MonitorInforAction extends CrudActionSupport<MonitorInfor> {
	...
	public String tree() throws Exception {
		...
		for (BusinessSystem businessSystem : businessSystems) {
			ZTreeNode node = new ZTreeNode("root_"+businessSystem.getCode(),"0",businessSystem.getName(), "true", "false", "", "", "folder", "");
			ztreeNodes.add(node);
...
//	
	private void children(BusinessSystem businessSystem,List<ZTreeNode> zTreeNodes){
		ZTreeNode node = new ZTreeNode(businessSystem.getCode()+ "_http","root_"+businessSystem.getCode(),"http监控", "true", "false", "", "", "folder", "");
		zTreeNodes.add(node);
		node = new ZTreeNode(businessSystem.getCode()+ "_jdbc","root_"+businessSystem.getCode(),"jdbc监控", "true", "false", "", "", "folder", "");
		zTreeNodes.add(node);
		node = new ZTreeNode(businessSystem.getCode()+ "_meth","root_"+businessSystem.getCode(),"meth监控", "true", "false", "", "", "folder", "");
		zTreeNodes.add(node);
