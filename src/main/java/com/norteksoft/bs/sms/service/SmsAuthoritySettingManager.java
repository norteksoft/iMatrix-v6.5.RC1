...
public class SmsAuthoritySettingManager {
	...
	public void save(Long systemId, String requestType, SmsAuthoritySetting smsAuthoritySetting) {
		...
		if("接收".equals(smsAuthoritySetting.getType())){
	...
	public String changeStatus(String ids) {
		...
		sb.append("草稿到启用:").append(draftToEnableNum).
			append(";禁用到启用:").append(disableToEnableNum).
			append(";启用到禁用:").append(enableToDisableNum);
		return sb.toString();
	}
