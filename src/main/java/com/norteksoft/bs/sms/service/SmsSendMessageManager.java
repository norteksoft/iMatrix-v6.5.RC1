...
public class SmsSendMessageManager {
	...
	private void sendMessage(SmsGatewaySetting smsGatewaySetting,List<SmsWaitTosend> waitTosends) throws Exception{
		for (SmsWaitTosend smsWaitTosend : waitTosends) {
			...
			if(null == sendMsgImpl){
				ApiFactory.getBussinessLogService().log("短信平台", "短信平台发送短信,没有取到实现类,网关编号:" +
						smsGatewaySetting.getGatewayCode(),ContextUtils.getSystemId("bs"));
			}else {
				...
				if("success".equals(flag)){
					...
					ApiFactory.getBussinessLogService().log("短信平台", "短信平台发送短信,发送失败,网关编号:" +
					   smsWaitTosend.getId()+",网关:"+smsGatewaySetting.getGatewayCode(),ContextUtils.getSystemId("bs"));
				}
			}
		}
	}
	...
	private String httpClientConnection(String url) {
		...
		ApiFactory.getBussinessLogService().log("短信平台", "短信平台发送短信,http方式访问回调,url：" +
					url+",返回值:"+responseBody,ContextUtils.getSystemId("bs"));
	...
	private String restfulConnection(String url) {
		...
		ApiFactory.getBussinessLogService().log("短信平台", "短信平台发送短信,restful方式访问回调,url：" +
				url+",返回值:"+cr.getEntity(String.class),ContextUtils.getSystemId("bs"));
