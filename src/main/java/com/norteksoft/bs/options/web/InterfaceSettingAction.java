...
public class InterfaceSettingAction extends CrudActionSupport<InterfaceSetting> {
	...
	public String delete() throws Exception {
		...
		ApiFactory.getBussinessLogService().log("普通接口管理", "删除普通接口", ContextUtils.getSystemId("bs"));
	...
	public String input() throws Exception {
		ApiFactory.getBussinessLogService().log("普通接口管理", "普通接口表单页面", ContextUtils.getSystemId("bs"));
	...
	public String view() throws Exception {
		ApiFactory.getBussinessLogService().log("普通接口管理", "查看普通接口", ContextUtils.getSystemId("bs"));
	...
	public String list() throws Exception {
		if(page.getPageSize()>1){
			ApiFactory.getBussinessLogService().log("普通接口管理", "普通接口列表", ContextUtils.getSystemId("bs"));
	...
	public String save() throws Exception {
		...
		ApiFactory.getBussinessLogService().log("普通接口管理", "保存普通接口", ContextUtils.getSystemId("bs"));
		return "interface-input";
	}
	/**
	 * 启用/禁用普通接口
	...
	public String changeInterfaceState() throws Exception {
		...
		ApiFactory.getBussinessLogService().log("普通接口管理", "启用/禁用普通接口", ContextUtils.getSystemId("bs"));
		return null;
	}
	/**
	 * 左侧的三级菜单定时和接口管理下的树
	...
	public String createInterfaceTree()throws Exception {
		...
		ZTreeNode root = new ZTreeNode("_interface","0", "普通接口", "false", "false", "", "", "folder", "");
		treeNodes.add(root);
		root = new ZTreeNode("_job_time","0", "定时接口", "true", "false", "", "", "folder", "");
		treeNodes.add(root);
		
		for(BusinessSystem system :businessSystems){
			root = new ZTreeNode(system.getId().toString(),"_job_time", system.getName(), "true", "false", "", "", "folder", "");
			treeNodes.add(root);
		}
