...
//	}
	...
	function delTransactor(btnId){
		...
		if(ids==''){
			...
			var state=rowData['processState'];
			if(state=="SUBMIT"){
				$.colorbox({href:webRoot+'/engine/task-delTransactor.htm?instanceId='+workflowId,iframe:true, width:400, height:500,overlayClose:false,title:"选择"});
			}else{
	...
	function openOverdueTaskDetail(transactorId,transactor,position){
		...
		if(transactorId!="&nbsp;"&&typeof(transactorId)!="undefined"&&transactorId!=""&&transactorId!=null){
			init_colorbox(webRoot+"/engine/workflow-definition-monitor-task-list-user.htm?transactorId="+transactorId+"&"+searchPar,"详情");
		}else{
			init_colorbox(webRoot+"/engine/workflow-definition-monitor-task-list-user.htm?transactor="+transactor+"&"+searchPar,"详情");
		}
	}
	//格式化“是否移交”
	...
	function openTransferTaskDetail(taskId,position){
		init_colorbox(webRoot+"/engine/workflow-definition-monitor-task-transfer.htm?taskId="+taskId+"&position="+position,"移交任务列表");
	}
	//格式化任务移交数
	...
	function openTransactorTransferTask(transactorId,position){
		var searchPar = getSearchParam();
		init_colorbox(webRoot+"/engine/workflow-definition-monitor-transfer-task-list-user.htm?transactorId="+transactorId+"&"+searchPar,"移交任务列表");
	}
