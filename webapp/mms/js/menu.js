...
function createMenu(){
	...
	if(currentNode.data.indexOf('placeholder_')==0){
		$("#message").html("<font class=\"onError\"><nobr>不能在占位符下新建菜单</nobr></font>");
		location.hash="anchor";
		showMsg();
		return;
	}
	if(currentNode.data=='portal'){
		$("#message").html("<font class=\"onError\"><nobr>不能在门户下新建菜单</nobr></font>");
		location.hash="anchor";
		showMsg();
		return;
	}
	...
	$.colorbox({href:webRoot+"/module/menu-input.htm?parentMenuId="+currentMenuId+"&selectPlaceholderSign="+selectPlaceholderSign,iframe:true, innerWidth:500, innerHeight:410,overlayClose:false,title:"创建菜单"});
}
...
function createSystem(){
	...
	$.colorbox({href:webRoot+"/module/menu-input.htm?parentMenuId="+currentMenuId+"&isCreateSystem="+isCreateSystem,iframe:true, innerWidth:500, innerHeight:380,overlayClose:false,title:"创建系统"});
}
//判断是否选择一个菜单
function isSelected(){
	if(!currentMenuId||typeof(currentMenuId)=="undefined"){
	...
	if(isSelected()){
		...
		$.get(webRoot+"/module/menu-input.htm?menuId="+currentMenuId+"&_data="+new Date(),function(data){
			if(data=="false"){
				...
				$.colorbox({href:webRoot+"/module/menu-input.htm?menuId="+currentMenuId+"&selectPlaceholderSign="+selectPlaceholderSign,iframe:true, innerWidth:500, innerHeight:410,overlayClose:false,title:"修改菜单"});
			}
		});
	}
}
