...
<head>
	...
	function isWidgetExist(){
		$.ajax({
			   ...
			   success: function(msg){
				   if(msg=="true"){
					  alert("<s:text name='window_Numbers_already_exist'/>");
				   }else{
					   $("#inputForm").submit();
				   }
			   }
	...
	function updateWiget(){
		...
		if(ids.length<=0){
			alert("<s:text name='Please_choose_the_need_edit_record'/>！");
			return;
		}else if(ids.length>1){
			alert("<s:text name='doNot_select_multiple_records'/>！");
			return;
		}else{
	...
	function validateDeleteWidget(){
		var ids=jQuery("#widgetPage").getGridParam('selarrrow');
		if(ids.length<=0){
			alert("<s:text name='select_need_to_delete_record'/>！");
			return;
		}
			   ...
			   success: function(msg){
				   ...
				   if(msg==""){
					  deleteWiget();
					   ...
					   if(confirm(msg+"<s:text name='form_being_used_sure_delete'/>?")){
						   deleteWiget();
					   }
				   }
			   }
