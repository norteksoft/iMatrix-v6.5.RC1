	...
	<aa:zone name="main_zone">
		...
		<div class="opt-btn">
			...
			<button class="btn" onclick="<s:if test="standardField=='~~roleId'||standardField=='~~subCompanyId'">select();</s:if><s:else>selectMans();</s:else>"><span><span >确定</span></span></button>
			 -->
			<button class="btn" onclick="enteringValue();"><span><span >确定</span></span></button>
		</div>
		<div id="opt-content">
			<div id="tabs-0" style="margin-left: 5px">
				条件值类型：<select id="selectConditionValueType" onchange="changeConditionValueType(this);" style="width: 150px;">
					<option value="entering">手工输入/选择</option>
					<s:if test="dataType=='TEXT' || dataType=='LONG'">
					<option value="default">标准值</option>
					</s:if>
					<option value="beanName">动态计算值</option>
				</select>
			</div>
			<div id="tabs-1" style="margin-left: 5px;margin-top: 10px;">
				<s:if test="values.size()>0">
					...
					条件值：<table class="form-table-border-left" style="width: 240px;margin-left: 5px;margin-top: 10px;">
			...
			<div id="tabs-3" style="display: none;margin-left: 5px;margin-top: 10px;">
				beanName：<input id="springBeanName" name="springBeanName"  style="width: 175px;"/>(提示：spring bean的名称)
			</div>
		</div>
		<script type="text/javascript">
