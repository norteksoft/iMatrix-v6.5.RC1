	...
	<script type="text/javascript">
    				...
    				success : function(data, textStatus) {
    					if(data=="loginNameFlase"){
    						...
    						if(!validateName($("#loginName").val())){
    			    			$("#loginName").parent().append('<label  class="error">登录名必填,登录名中不能包含除下划线(_)、横线(-)外的特殊字符！</label>');
    		    			}else  if(iMatrix.validateValueTrim(userName)){
    			    			$("#trueName1").parent().append('<label  class="error">真实姓名必填,真实姓名的前后不能包含空格！</label>');
		...
		*/
		...
		function copyRoleToUser(sourceIds){
			...
			if(confirm("确认复制？(管理员权限不会被复制)")){
				$.ajax({
					   data:{uids:sourceIds,userIds:userIds},
					   type: "POST",
					   cache:false,
		...
		function copyRoleToDepartment(sourceIds){
			...
			if(confirm("确认复制？(管理员权限不会被复制)")){
				$.ajax({
					data:{uids:sourceIds,deptIds:deptIds},
					type: "POST",
					cache:false,
		...
		function copyRoleToWorkgroup(sourceIds){
			...
			if(confirm("确认复制？(管理员权限不会被复制)")){
				$.ajax({
					   data:{uids:sourceIds,workgroupIds:workgroupIds},
					   type: "POST",
					   cache:false,
				...
				<security:authorize ifAnyGranted="copy_authority">
							<div class="btndiv" id="_copyAuthorize" style="*top:-2px;">
								<button  class="ui-button ui-widget ui-state-default ui-button-text-only ui-corner-left" id="parentCopyBtn" onclick="userTree()">
									<span class="ui-button-text">复制权限给人员</span>
								</button>
							...
							<div style="width: 130px" id="copyAuthorize" class="flag" >
								<ul style="width: 130px" >
									<li ><a href="#"  onclick="departmentTree()">复制权限给部门</a></li>
									<li ><a href="#"  onclick="workgroupTree()">复制权限给工作组</a></li>
								</ul>
							</div>
				</security:authorize>
							...
							<div id="updatebtn" class="flag" >
								<ul >
							        ...
							        <security:authorize ifAnyGranted="acs-organization-user-unlockData">
											<li ><a href="#" onclick="unlockData();">数据解锁</a></li>
									</security:authorize>
								</ul>
							</div>
							<script type="text/javascript">
							...
							<div id="exportbtn" class="flag" >
								<ul>
									<security:authorize ifAnyGranted="acs_user_exportUser">
										<li><a href="#" onclick="exportUser();">导出</a></li>
									</security:authorize>
								</ul>
							</div>
						</security:authorize>
